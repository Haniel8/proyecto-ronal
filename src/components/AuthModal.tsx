...
export default function AuthModal({
  ...
  const handleSubmit = async () => {
    ...
    const displayName = mode === "login" ? email.split("@")[0] : name;
          ...
          <div className="flex flex-col gap-3 flex-1">
            ...
            <div className="flex gap-3">
              {["Google", "Facebook"].map((provider) => (
                <button
                  key={provider}
                  onClick={() => { login(provider + "User", `user@${provider.toLowerCase()}.com`); onClose(); onSuccess?.(); }}
                  className="flex-1 flex items-center justify-center gap-2 border border-gray-200 hover:bg-gray-50 text-gray-600 text-sm font-medium py-2.5 rounded-2xl transition-colors"
                >
                  {provider === "Google" ? (
